...
Формат — шаблон v2: 14 колонок, колонки СДР нет. Иерархия читается только из
...
# Колонка 15 — надстройка над результатом расчёта (DEC-31, standards.md §17а).
# В 14 колонок шаблона v2 не входит и в MS Project не импортируется.
...
def validate(tasks: list[dict], reserve_base: datetime | None = None,
    ...
    r.check(not missing_cols, "выведены все 14 колонок формата v2", f"нет: {missing_cols}")
    allowed_constraints = {"", "Как можно раньше", "Начало не ранее"}
    bad_constraints = [label(t) for t in tasks
                       if t.get("Тип ограничения", "") not in allowed_constraints]
    r.check(not bad_constraints, "типы ограничений допустимы",
            f"неизвестные: {bad_constraints[:8]}")
    snet_without_start = [label(t) for t in tasks
                          if t.get("Тип ограничения") == "Начало не ранее"
                          and not t.get("Начало")]
    r.check(not snet_without_start, "у «Начало не ранее» задана дата",
            f"без даты: {snet_without_start[:8]}")
