...
class Std:
    ...
    ROOF_STYLOBATE = (90, "standards.md §11 STD-KRV-003")
    ...
    TECH_ROOMS = (60, "standards.md §15 STD-OTD-003, DEC-08")
    ...
    BIO = {
        ...
        "STD-BIO-002": (120, "Твёрдые покрытия"),
        "STD-BIO-003": (60, "Озеленение"),
        "STD-BIO-004": (45, "Ограждение"),
        "STD-BIO-005": (45, "Наружное освещение"),
        "STD-BIO-006": (45, "МАФ"),
        "STD-BIO-007": (45, "Организация дорожного движения"),
        "STD-BIO-008": (60, "Резерв блока БИО (DEC-13, явная строка)"),
    }
...
class Bnd:
    ...
    LAG_ANCHOR_PVC = (30, "bindings.md §1.2, ЯКОРЬ_ОГР НН +30 → СПК ПВХ")
    LAG_PVC_FACADE = (30, "bindings.md §1.2, старт СПК ПВХ НН +30 → старт фасада")
    LAG_ANCHOR_FIT = (90, "bindings.md §1.2, ЯКОРЬ_ОГР НН +90 → старт отделки")
    LAG_HEAT = (15, "bindings.md §3.5 BND-TC-003, TC_eff +15 → пуск тепла")
    LAG_ITP = (90, "bindings.md §3.6 BND-VIS-006, старт перегородок +90 → ИТП")
    LAG_TECH_ROOMS = (21, "bindings.md BND-VIS-004, старт перегородок +21")
    LAG_BIO = (180, "bindings.md §3.8 BND-BIO-001, DEC-22 — ОН −180 от фасадов")
    FIT_TARGET = (365, "bindings.md §1.2 / §3.7 BND-OTD-003, целевой финиш ЯКОРЬ +365")

    # Финиши, привязанные к ЯКОРЬ
    FIN_MASONRY_EXT = (45, "standards.md §12 STD-KLD-001, финиш ЯКОРЬ +45")
    FIN_MASONRY_INT = (120, "standards.md §12 STD-KLD-002, финиш ЯКОРЬ +120")
    FIN_PVC = (75, "bindings.md §3.3 BND-SPK-001, финиш ЯКОРЬ +75 при наличии кладки")
    FIN_PVC_NO_MASONRY = (120, "bindings.md §3.3 BND-SPK-001, финиш ЯКОРЬ +120 без кладки")
...
# ======================================================================
# 3. Расчёт корпуса — bindings.md §2, двенадцать шагов
# ======================================================================
...
def compute_thermal_and_fit(
    ...
    if facade_type.lower() in ("модульный", "панельный"):
        ...
        a.append(Assumption("typGRP §14 п.5",
    ...
    s0 = anchor_ogr + timedelta(days=Bnd.LAG_ANCHOR_FIT[0])
    year = s0.year
    tr.append(f"S0 = ЯКОРЬ_ОГР + 90 = {dfmt(s0)}; год отделки Y = {year} "
              f"(bindings.md §2, разрыв круговой зависимости — берётся до гейта)")

    # --- ИТП (BND-VIS-006) -----------------------------------------------
    itp_start = partitions_start + timedelta(days=Bnd.LAG_ITP[0])
    itp_finish = itp_start + timedelta(days=Std.ITP[0])
    tr.append(f"ИТП: старт = старт перегородок + 90 = {dfmt(itp_start)}, "
              f"+{Std.ITP[0]} дн → финиш {dfmt(itp_finish)} ({Std.ITP[1]})")
    if not has_parking:
        a.append(Assumption("DEC-06", "Паркинг отсутствует — ИТП развёрнут в каждом корпусе. "
    ...
    def gate(heat_date: date) -> date:
        ...
        deadline = date(year, *Bnd.DEADLINE_HEAT)
        if heat_date > deadline:
