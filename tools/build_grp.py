...
class Build:
    ...
    def load_skeleton(self) -> None:
        ...
        # Корневая строка «0» «Наименование проекта» в выдачу не включается —
        # решение владельца 31.07.2026. Ссылок на неё в эталоне нет, разделы
        # 1…18 и без неё стоят на уровне 1, иерархия не нарушается.
        raw = [r for r in raw if r["СДР"] != "0"]
    ...
    def finalize(self, nodes: dict[str, Node]) -> list[dict]:
        ...
        for i, r in enumerate(self.rows, start=1):
            ...
            out.append({
                ...
                # Длительность и предшественники у суммарных строк остаются
                # пустыми — именно они ломают импорт (typGRP.md §2.2 п.4).
                # Даты выводятся: решение владельца 31.07.2026, свёрнуты из
                # потомков (старт = минимум, финиш = максимум).
