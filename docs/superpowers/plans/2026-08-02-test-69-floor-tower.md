...
2. **Валидатор — ноль ошибок** на обоих входах: `python tools/validate_grp.py out/ГРП_эталон.xlsx` и `python tools/validate_grp.py out/ГРП_69.xlsx`. Код возврата 0. Предупреждения («ВНИМАНИЕ») ошибками не считаются.
