...
class Build:
    ...
    def configure_zero_cycle(self) -> None:
        ...
        for r in self.rows:
            ...
            if changed:
                # Внешняя строка шаблона может ссылаться сразу на прототипы К1
                # и К2. Каждый такой прототип разворачивается на все корпуса,
                # поэтому без дедупликации одна физическая связь появляется
                # повторно. Порядок сохраняем, разные типы и лаги не склеиваем.
    ...
    def configure_parking(self) -> None:
        ...
        # ИТП, отопления паркинга и наружного теплоснабжения); при встроенном
        # паркинге объекта «паркинг» в проекте нет, его подземные уровни — часть
